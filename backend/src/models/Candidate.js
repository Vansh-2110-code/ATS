...
const CANDIDATE_STATUSES = [
  // 1 to 17: Recruiter & TL
  'Eligible', 'Not Eligible', 'No Response', 'Duplicate-Client', 'Call Back',
  'Hold', 'Submitted to Client', 'Sublitted To Client', 'Submitted To Client', 'Walkin Company', 'Walkin WHM',
  ...
  // 18 to 31: TL & Management
  'L1 Select', 'L1 Reject', 'L2 Select', 'L2 Reject',
  'Final Select', 'Final Reject', 'Document Initialized',
  ...
  'Did Not Pick', 'Unanswered Calls', 'HR Shortlist', 'Written Test',
  'Operations Round', 'Document Pending', 'Documentation', 'Yet To Join',
  'Walk-in Submitted', 'Exited', 'Not Interested'
];
...
const candidateSchema = new mongoose.Schema({
  ...
  offerDetails: {
    ...
    placementPercentage: { type: Number, default: 8.33 },
...
candidateSchema.pre('save', async function(next) {
  ...
  if (this.isModified('status')) {
    const s = this.status;
    if (['Shortlisted', 'Screening'].includes(s)) this.currentStage = 'Screening';
    else if (['Interview', 'Selected'].includes(s)) this.currentStage = 'Interview';
    else if (['Offered'].includes(s)) this.currentStage = 'Offer';
...
// Auto-calculate revenueGenerated on update (handles both $set and plain object updates)
