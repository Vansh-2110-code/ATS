    ...
    if (activeView === 'active-jr') {
      ...
      (teamWiseData.teams || []).forEach(team => {
        ...
        (team.members || []).forEach((m: any) => {
          rows.push(`"  ΓööΓöÇ ${m.name}","${m.employeeId}","Recruiter",${m.submitted},${m.selects},${m.joinees},"${m.joineesVsSubmittedRatio}","${m.joineesVsSelectsRatio}"`);
        });
      });
    ...
    <div className="p-6 max-w-6xl mx-auto space-y-5">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        {[
          ...
          { label: 'Avg Conv. Rate', value: performanceData.length ? (performanceData.reduce((s: number, r: any) => s + parseFloat(r.convRate || '0'), 0) / performanceData.length).toFixed(1) + '%' : 'ΓÇö', color: 'amber' },
        ].map((s, i) => {
      ...
      <div className="bg-white rounded-xl border border-slate-100 shadow-sm p-5">
        <h3 className="text-slate-800 text-sm mb-4" style={{ fontWeight: 600 }}>Monthly Overview ΓÇö Calls vs Placements</h3>
                        ...
                        <td colSpan={8} className="p-4">
                          <div className="bg-white rounded-lg border border-slate-200 p-4 space-y-3">
                            ...
                            {j.activeCandidates.length === 0 ? (
                              <p className="text-xs text-slate-400">No candidates in pipeline for this JR.</p>
                            ) : (
                                ...
                                <table className="w-full text-[11px]">
                                  <thead>
                                    <tr className="bg-slate-50/50 text-slate-500 text-left border-b border-slate-100">
                                      ...
                                      <th className="px-3 py-2 font-semibold">Assigned Recruiter</th>
                                  ...
                                  <tbody className="divide-y divide-slate-50 text-slate-600">
                                    {j.activeCandidates.map((c: any) => (
                                      <tr key={c._id} className="hover:bg-slate-50/30">
                                        ...
                                        <td className="px-3 py-2">{c.recruiterName || 'Unassigned'}</td>
                                        <td className="px-3 py-2">{new Date(c.updatedAt).toLocaleDateString()}</td>
                                      </tr>
                                    ))}
                                  </tbody>
                                </table>
        ...
        <div className="flex flex-wrap gap-1.5">
          {['All', 'Documentation', 'Pending Customer', 'Yet To Join', 'Screening'].map(flt => (
            <button
              ...
              className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-all ${activeProfileFilter === flt
              ...
              {filteredActiveProfiles.length === 0 ? (
                ...
                filteredActiveProfiles.map((c, idx) => (
                  <tr key={idx} className="hover:bg-slate-50/60 transition-colors">
                    ...
                    <td className="px-4 py-3.5 text-center">
                      <span className={`px-2 py-0.5 rounded font-semibold ${c.daysPending > 14 ? 'bg-red-100 text-red-700' :
      ...
      <div className="bg-white rounded-xl border border-slate-100 shadow-sm p-4 flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex gap-2">
          <button
            ...
            className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-all cursor-pointer ${revenueSubView === 'joined-candidates' ? 'bg-emerald-600 text-white shadow-sm' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'
              }`}
          >
          ...
          <button
            ...
            className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-all cursor-pointer ${revenueSubView === 'customer' ? 'bg-violet-600 text-white shadow-sm' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'
              }`}
          >
            Customer Breakdown
          </button>
          <button
            ...
            className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-all cursor-pointer ${revenueSubView === 'division' ? 'bg-violet-600 text-white shadow-sm' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'
              }`}
          >
            Division Breakdown
          </button>
                ...
                {((expectedRevenueData as any).joinedCandidates || []).length === 0 ? (
                  ...
                  ((expectedRevenueData as any).joinedCandidates || []).map((c: any, i: number) => (
                    <tr key={c._id || i} className="hover:bg-slate-50/60 transition-colors">
                      ...
                      <td className="px-5 py-3.5 text-right font-semibold text-slate-800">{c.ctc ? fmt(c.ctc) : 'ΓÇö'}</td>
      ...
      <div className="bg-white rounded-xl border border-slate-100 shadow-sm p-4 flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex gap-2">
          <button
            ...
            className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-all cursor-pointer ${leadPerfMode === 'team-wise' ? 'bg-amber-600 text-white shadow-sm' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'
              }`}
          >
          ...
          <button
            ...
            className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-all cursor-pointer ${leadPerfMode === 'flat' ? 'bg-slate-700 text-white shadow-sm' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'
              }`}
          >
                            ...
                            team.members.map((m: any) => (
                              <tr key={m.userId} className="hover:bg-slate-50/70 transition-colors">
                                <td className="px-5 py-3.5 font-bold text-slate-900 flex items-center gap-2">
                                  <span className="text-slate-400 font-mono text-[10px]">ΓööΓöÇ</span> {m.name}
                                </td>
...
{/* ΓöÇΓöÇΓöÇ VIEW: CUSTOMER WISE ΓöÇΓöÇΓöÇ */ }
{
  activeView === 'customer' && (
    <div className="bg-white rounded-xl border border-slate-100 shadow-sm overflow-hidden">
...
{/* ΓöÇΓöÇΓöÇ VIEW: DIVISION WISE ΓöÇΓöÇΓöÇ */ }
{
  activeView === 'division' && (
    <div className="bg-white rounded-xl border border-slate-100 shadow-sm overflow-hidden">
      <div className="px-5 py-4 border-b border-slate-100 flex justify-between items-center">
...
{/* ΓöÇΓöÇΓöÇ VIEW: AGING REPORT ΓöÇΓöÇΓöÇ */ }
{
  activeView === 'aging' && (
    <div className="space-y-6">
              ...
              {(!agingData.candidates || agingData.candidates.length === 0) ? (
                ...
                agingData.candidates.map((c: any, i: number) => (
                  <tr key={i} className="hover:bg-slate-50/50">
                    ...
                    <td className="px-5 py-3.5">
                      <span className={`px-2 py-0.5 text-xs rounded font-semibold ${c.daysPending > 15 ? 'bg-red-50 text-red-600' :
...
{/* ΓöÇΓöÇΓöÇ VIEW: CONVERSION RATIO ΓöÇΓöÇΓöÇ */ }
{
